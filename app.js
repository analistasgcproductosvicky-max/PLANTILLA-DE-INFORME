...
// Recopilar datos de UNA sección (scope = elemento raíz de la sección)
function recopilarSeccion(secId) {
  const root = secId === 'general'
  ...
  // Si/No
  const sinos = {};
  root.querySelectorAll('.sino-row').forEach(row => {
  ...
  // Responsables
  const resp = {};
  root.querySelectorAll('[id^="resp-"]').forEach(tags => {
    resp[tags.id] = [...tags.querySelectorAll('.resp-tag')].map(t => t.dataset.nombre);
  });
  d._resp = resp;
  // Fotos
  const fotos = {};
  root.querySelectorAll('.fgrid').forEach((grid, i) => {
    if(!grid.dataset.fid) grid.dataset.fid = secId + '_fg' + i;
  ...
  // Tablas
  const tablas = {};
  root.querySelectorAll('table.reg[id]').forEach(t => {
    tablas[t.id] = [...t.querySelectorAll('tbody tr')].map(tr =>
      [...tr.querySelectorAll('input,select')].map(el => el.value)
    );
  });
  d._tablas = tablas;
  // Botones activos (rbtn.si que no son sino-row)
  const btns = {};
  root.querySelectorAll('.rbtn.si:not(.sino-row .rbtn)').forEach(b => {
    if(b.id) btns[b.id] = true;
  });
  d._btns = btns;
  return d;
}
...
function recopilarTodasSecciones() {
  const secciones = { general: recopilarSeccion('general') };
  document.querySelectorAll('.seccion[id]').forEach(sec => {
    const secId = sec.id.replace('sec-','');
    secciones[secId] = recopilarSeccion(secId);
  });
  return secciones;
}

// Restaurar UNA sección
function restaurarSeccion(secId, d) {
  if(!d) return;
  const root = secId === 'general'
    ? document.getElementById('form-content')
    : document.getElementById('sec-' + secId);
  ...
  if(d._sinos) {
    root.querySelectorAll('.sino-row').forEach(row => {
      ...
      if(btn) {
        btn.classList.add(val);
        const key2 = row.dataset.key;
        const pregEl2 = document.getElementById('preg-'+key2) || row.closest('.pregunta');
        if(pregEl2) {
          const ccs = pregEl2.querySelector(`.cond-si[data-for="${key2}"]`);
          const ccn = pregEl2.querySelector(`.cond-no[data-for="${key2}"]`);
          if(ccs) ccs.style.display = val==='si'?'block':'none';
          if(ccn) ccn.style.display = val==='no'?'block':'none';
        }
      }
    });
  }
  ...
  if(d._tablas) {
    Object.entries(d._tablas).forEach(([tid, filas]) => {
      ...
      while(tb.rows.length < filas.length) {
        if(isTolvas) addRowTolvas(tid); else addRow(tid, cols);
      }
      [...tb.rows].forEach((tr, ri) => {
        ...
        if(isTolvas) { const di=tr.querySelectorAll('input')[2]; if(di) calcDiasTolva(di); }
      });
    });
  }
  // Botones activos
  if(d._btns) {
    Object.keys(d._btns).forEach(id => {
...
function _abrirFormularioNuevo(tipo) {
  ...
  if(tipo === 'emp') {
    ...
    renderEmpaque();
    if(borradorId && window._fb) setTimeout(iniciarRealtime, 500);
  } else if(tipo === 'pe') {
